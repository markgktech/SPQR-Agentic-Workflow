...
The recording target is the repo, not central memory: decisions → the run's PoC (poc/ lane); briefs, Changes Made and run state → the run container (docs/spqr_self/upgrades/<version>/).
Before compacting: verify that the run's PoC captures all open decisions and the run container reflects current state.
...
The run container (with its linked PoC) is the external record — the master session can be reconstructed from it if context is lost.
...
COMPACT PROTOCOL
1. Record any unsaved decisions in the run's PoC first.
...
NEVER
- Compact without recording decisions in the run's PoC first
- Compact without a context note in /compact args
- Re-derive decisions already documented in the run's PoC
