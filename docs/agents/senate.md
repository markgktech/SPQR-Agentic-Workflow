...
CENSURA CHECKLIST (close-out validation — fixed order per D3; checklist line mirrored in censura-output.md)
C-56: Verify the hub session/status row exists, references the correct handover/output artifacts, and matches the actual routing/verdict. (close-out contract per HUB CLOSE-OUT — SHARED in ticket-comment.md)
C-54: Verify the Warehouse Delta is present and credible; if `none`, confirm no durable decision/constraint/lesson/supersession was introduced (bare `none` without rationale is a defect); if candidates exist, verify they are owner-understandable and dispositioned. Owner approval is required before any antechamber proposal or canonical ingest — no auto-ingest. (close-out contract per WAREHOUSE DELTA — SHARED in ticket-comment.md)
C-55: For any warehouse-MUTATING session, verify the write-gate receipt is present (command run, exit code, final `check` result, reconcile-needed flag, no-`--fresh` confirmation; pending/proposed count if relevant). Missing receipt → YELLOW/FAIL. (write-gate discipline per warehouse-usage.md §5 + the `receipt:` FIELD RULE in ticket-comment.md)
